...
<div <?php tfuse_class('middle'); ?>>
    <div class="container_12">
        ...
        <div class="content">
            ...
            if (!is_wp_error($location) && ($TFUSE->request->isset_GET('location_id') || $TFUSE->request->isset_GET('main_location'))) {
            ...
            <div class="title">
                <h1><?php
                    if ($TFUSE->request->isset_GET('category_ids') && ($TFUSE->request->GET('category_ids')) && (!strpos(',', $TFUSE->request->GET('category_ids')))) {
                        $cat_id = intval($TFUSE->request->GET('category_ids'));
                        $cat = get_term($cat_id, TF_SEEK_HELPER::get_post_type() . '_category');
                        if ($cat && (isset($cat->name)))
                            $cat_name = $cat->name;
                        else
                            $cat_name = TF_SEEK_HELPER::get_option('seek_property_name_plural', 'Holidays'); echo mb_strtoupper($cat_name, 'UTF-8');
                    } else {
                        echo mb_strtoupper(TF_SEEK_HELPER::get_option('seek_property_name_plural', 'Holidays'), 'UTF-8');
                    }
                    ?> <?php
                    if (!$location_error) {
                        echo ' - ';
                    ...
                    <span><?php if (!$location_error) echo $location; ?></span><?php ?>
            ...
            <div class="re-list">
                        ...
                        <div class="re-item">
                            <div class="re-image">
        <?php if ($spost['seek_property_reduction'] != 0) : ?><span class="ribbon off-<?php echo $spost['seek_property_reduction']; ?>"><?php
            _e('SALE:', 'tfuse');
            echo $spost['seek_property_reduction'];
            _e('% OFF', 'tfuse');
            ?></span> <?php endif; ?>
                            ...
                            <div class="re-short">
                                <div class="re-top">
                                    ...
                                    <div class="re-subtitle"><?php
                                        if (!is_wp_error($categories) && isset($categories[0])) : echo ucfirst(strtolower($categories[0]->name));
                                            _e(':', 'tfuse');
                                        endif;
                                        ?>
                                        <?php
                                        ...
                                        ?>
                                        <?php
                                        if ($spost['seek_property_sale_type'] == 2) : _e(' (', 'tfuse');
                                            echo tfuse_page_options('during', '1', $spost['ID']);
                                            _e(' nights', 'tfuse');
                                            _e(')');
                                        endif;
                                        ?>
                        ...
                        echo $spost['seek_property_price'];
                        if ($spost['seek_property_sale_type'] == 1)
                            echo $price_suffix;
                        ?></strong></span>
...
<?php else: ?>
    ...
    if ($search_results['total'] < 1) {
        if (!$TFUSE->request->isset_GET('favorites'))
            _e('Sorry, but nothing matched your search criteria.', 'tfuse');
        else
            _e('Sorry, no favorites added yet', 'tfuse');
    } else {
        // Wrong page
    }
    ?>
