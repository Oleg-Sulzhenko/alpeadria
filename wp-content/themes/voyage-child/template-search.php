...
<div class="content">
    ...
    ?>
    <div class="title">
        <h1><?php if($TFUSE->request->isset_GET('category_ids') && ($TFUSE->request->GET('category_ids')) && (!strpos(',',$TFUSE->request->GET('category_ids')))) { $cat_id = intval($TFUSE->request->GET('category_ids')); $cat = get_term($cat_id, TF_SEEK_HELPER::get_post_type() . '_category'); if($cat && (isset($cat->name))) $cat_name = $cat->name; else $cat_name = TF_SEEK_HELPER::get_option('seek_property_name_plural','Holidays'); echo mb_strtoupper($cat_name, 'UTF-8'); } else {echo mb_strtoupper(TF_SEEK_HELPER::get_option('seek_property_name_plural','Holidays'), 'UTF-8'); }?> <?php  _e('для', 'tfuse'); ?> <span><?php if (!$location_error) echo $location; else _e('All Locations', 'tfuse'); ?></span><?php ?></h1>
